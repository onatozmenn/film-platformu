...
- Activation gate: GitHub Actions [CI run 29675271105](https://github.com/onatozmenn/film-platformu/actions/runs/29675271105) passed every job and non-skipped step for exact commit `03dc273e7327fe48f9fb14e2442d2b22fcdeaa19` before WP-07 implementation began.
- Repository-controlled release gates: production readiness is a pure ordered policy plus coarse CLI report; approved content remains null/404 until reviewed; CSP enforcement and HSTS require explicit public-HTTPS production switches plus evidence; structured logs carry an immutable release ID and cannot be context-spoofed; Vercel Cron uses the corrected authenticated `GET` transport with bounded/idempotent repositories; automatic CI adds tracked-secret and high/critical dependency scans; manual protected workflows reuse full CI, create a pinned CycloneDX SBOM, separate migration authority, and verify isolated restores.
- Database/content evidence: aggregate production checks require at least one currently publishable title, reject active supported-territory allow-rights without internal evidence, detect restored deletion/publication/admin inconsistencies, and reject runtime identities that are superusers, table owners, or schema creators. The checks expose only owned issue codes and aggregate counts.
- Security dependency repair: high-severity Nodemailer advisory `GHSA-p6gq-j5cr-w38f` was removed by upgrading to `9.0.1`; SMTP contracts and strict types pass, pnpm reports no peer issues through the exact reviewed NextAuth compatibility allowance, and audit now reports three moderate findings with no high/critical advisory.
- Operational/privacy contract: [`docs/10-RELEASE-EVIDENCE.md`](10-RELEASE-EVIDENCE.md) defines protected workflows, every non-secret evidence reference, current data inventory, and hard blockers without presenting engineering controls as legal approval.
- Local validation on 2026-07-19: formatting, zero-warning lint, strict typecheck, Prisma/database state, pinned `actionlint`, tracked-secret scan, high/critical dependency audit, production build, and four-route budgets pass. Unit/component/route/provider/parser/release coverage passes with 453 tests across 94 files at 87.30% statements, 83.33% branches, 84.37% functions, and 87.46% lines; 52 PostgreSQL tests pass across 19 files with six migrations current. The four-project browser suite passes 64 tests with 52 intentional skips; public JavaScript remains 172.9-174.6 KB gzip and CSS 11.5 KB. The separate staging suite discovers four read-only mobile/desktop cases and remains unexecuted until a real protected staging origin exists.
- Remote validation: GitHub Actions [CI run 29676886035](https://github.com/onatozmenn/film-platformu/actions/runs/29676886035) passed every job and non-skipped step for exact commit `4210ac7a9a2b0fb92dc014fc0959989c82bd20d5`. Frozen install, tracked-secret scan, high/critical dependency audit, pinned workflow validation, six migrations, formatting, zero-warning lint, strict typecheck, 453 unit/component/route/provider/parser/release tests, 52 PostgreSQL tests, database state, production build, public-route budgets, and the 64-pass browser suite all passed. The repository-controlled WP-07 hardening is accepted; protected release-readiness, migration, restore, and staging workflows remain unrun until their owner-managed GitHub Environments, values, and external systems exist.
...
- Open-film demo tooling: [`src/content/open-film-catalog.json`](../src/content/open-film-catalog.json) records reviewed Blender-hosted source, license, rights, credit, and artwork facts for four titles. `pnpm content:ingest:open` is read-only by default; explicit `--apply` performs idempotent signed Mux ingest and atomic catalog publication. This improves the demonstrable catalog without claiming production owner approval.
